...
const socket_bedrock = io.connect('http://' + document.domain + ':' + location.port);
var msg = new SpeechSynthesisUtterance();

socket_bedrock.on('stream', function(response) {
    transcription += "Bedrock: " + response + "\n";
    $('#transcript').val(transcription);
...
let handleEventStreamMessage = function (messageJson) {
    ...
    if (results.length > 0) {
        if (results[0].Alternatives.length > 0) {
            ...
            $('#transcript').val(transcription + "You: " + transcript + "\n");
...
$('#stop-button').click(function () {
    closeSocket();
    toggleStartStop();
    socket_bedrock.emit('audio_test', transcription);
});
